...
// not for the cap).
// The visible output -- a one-page CV plus feedback -- is ~1,500 tokens, but
// gpt-oss is a reasoning model and its hidden reasoning tokens come out of the
// same budget, so 2,500 truncated real CVs. 6,000 leaves room for both while
// staying far below the old 16,000, which was not a budget but the absence of
// one. reasoning_effort (lib/groq.js) is the lever that actually reduces spend
// here; this is only the ceiling that stops a runaway.
...
const SYSTEM_PROMPT =
  ...
  '- THE SIX SECTION HEADINGS ARE FIXED. Emit them exactly as written, in this ' +
  'order, omitting any with no supplied facts: "## Profile", "## Education", ' +
  '"## Projects", "## Work Experience", "## Skills", "## Interests". No other ' +
  'wording and no additions — "Experience & Responsibility" and "Projects & ' +
  'Technical Activity" are wrong. Anything that does not fit one of the six ' +
  'belongs under the nearest of them.\n' +
  '- Inside Education, Projects and Work Experience, give every entry its own ' +
  'head line before its bullets: the title in bold on its own line, and where ' +
  'dates or an organisation were supplied, an em dash then those, e.g. ' +
  '"**Sales assistant, Tesco** — Jun 2025 to present". Then its "- " bullets. ' +
  'Never put the title inside a bullet, and never invent dates to fill the ' +
  'pattern — omit the dash and what follows it when none were supplied.\n' +
  '- No tables, blockquotes, code fences, or horizontal rules inside the CV.\n\n' +
  'SELF-CHECK before you output — the same checks this app\'s health score runs, ' +
  'so a CV you write should not fail them:\n' +
  '- One heading per section. Never emit the same section heading twice; merge ' +
  'everything that belongs under it.\n' +
  '- State each fact once. If something already appears under Projects, do not ' +
  'repeat it under Experience or Interests — cross-reference or leave it out.\n' +
  '- One date format throughout, chosen from what was supplied: "2019–2026", ' +
  '"Jun 2025 – present". Never mix formats and never write a vague duration ' +
  '("since last year") when dates were supplied; if none were, omit dates ' +
  'entirely rather than inventing or approximating them.\n' +
  '- Omit any contact field that is empty. Never emit a label with nothing after ' +
  'it ("Phone:" with no number).\n' +
  '- The Profile must contain the target role and the vocabulary of that field. ' +
  'A profile with no role-relevant terms is invisible in a recruiter search.\n' +
  '- Where an outcome, figure or tool WAS supplied for a project or job, it ' +
  'belongs in that bullet. Do not leave supplied evidence unused. Where it was ' +
  'not supplied, leave the bullet without one — never estimate.\n' +
  '- Where two supplied facts contradict each other (two different figures for ' +
  'the same thing, overlapping dates), keep them both out of the CV body and ' +
  'raise it in the feedback instead, naming both values. Never silently pick ' +
  'one, average them, or carry both through as if they agree.\n' +
  '- A job or project supplied WITHOUT dates still belongs on the CV — include ' +
  'it, leave the dates off, and name the missing dates in the feedback as a ' +
  'specific fix ("add the start and end months for the restaurant job"). Never ' +
  'invent or approximate a date to make the entry look complete, and never ' +
  'carry a vague duration ("since last year", "a while") into the CV: that ' +
  'phrase goes in the feedback as the thing to replace.\n' +
  '- Where a supplied note cannot be turned into an active-verb bullet without ' +
  'adding facts (it states a quality rather than an action — "good with ' +
  'numbers"), keep it out of the bullets and name it in the feedback, saying ' +
  'what the student would need to state for it to become one.\n\n' +
  'RECRUITER AND ATS LENS — what actually decides whether this CV is seen:\n' +
  'An ATS is a searchable database, not a scoring engine. It does not rank or ' +
  'reject CVs. Recruiters find candidates by running keyword searches against it, ' +
  'and the only automatic rejections come from knockout questions a human set ' +
  '(right to work, minimum grades, location, availability). So the job is to be ' +
  'FINDABLE in a keyword search and convincing in a ten-second human scan. Apply ' +
  'that as follows:\n' +
  '- Use the vocabulary of the target role for things the candidate ACTUALLY did. ' +
  'Re-labelling real experience in the words a recruiter searches is correct; ' +
  'claiming experience they do not have is not. "Worked the till" becomes ' +
  '"point-of-sale (POS) and cash handling"; "helped customers" becomes "customer ' +
  'service". Never introduce a tool, system or skill absent from the facts.\n' +
  '- Write an acronym and its full form together on first use — "Search Engine ' +
  'Optimisation (SEO)" — so a search for either term matches.\n' +

  '- Open the Profile with the target role in the words the advert would use, ' +
  'then the strongest concrete evidence. The first two lines are what a recruiter ' +
  'reads before deciding to keep reading.\n' +
  '- Make the Skills section exact-match terms, comma-separated, drawn only from ' +
  'the supplied facts. Short noun phrases, no sentences, no self-assessment ' +
  '("good communicator" is not a search term and no recruiter believes it).\n' +
  '- Surface knockout facts early when supplied — availability, right to work, ' +
  'driving licence, DBS, predicted or achieved grades. For school leavers these ' +
  'are what filters actually test. Omit entirely if not supplied.\n' +
  '- Put the most role-relevant section directly after Profile, whichever it is. ' +
  'A candidate with real projects and no jobs leads with Projects.\n\n' +
  'After the CV, output a line containing only "---", then a "## Feedback" section with:\n' +
  '- Short "- " bullets naming the weakest sections and why, written as a recruiter ' +
  'would say it: what they would doubt, and what they would look for and fail to find.\n' +
  '- Then the exact heading "### Search terms a recruiter would use that your CV ' +
  'does not contain" followed by 3-6 "- " bullets. Each names a term, then states ' +
  'plainly which it is: something they may already have done but described in other ' +
  'words (and what to reword), or something requiring real experience they do not ' +
  'yet have (and that it must be earned, not added). Never tell them to insert a ' +
  'term they cannot back up.\n' +
...
// A turn emits one question plus a rating and 2-3 sentences of feedback, and
// the final turn adds a short summary -- a few hundred tokens. 8,000 was not a
// budget, it was an absent ceiling: a runaway response could have spent 4% of
// the site's daily token allowance in one call.
const INTERVIEW_MAX_TOKENS = 4000;   // question + rating + feedback, plus reasoning tokens
...
const INTERVIEW_PROMPT =
  ...
  'OUTPUT — return ALL FIVE fields on EVERY turn; use null for any that do ' +
  'not apply. The schema does not enforce which combination is valid, so this ' +
  'is on you.\n' +
  'Opening turn (no answers yet): rating null, feedback null, done false, ' +
  'next_question the first question, summary null.\n' +
  'Middle turns: rating 1-10, feedback a string, done false, next_question ' +
  'the next question, summary null.\n' +
  'Final turn: rating 1-10, feedback a string, done true, next_question null, ' +
  'summary an object with overall 1-10, strengths, and work_on (three items).';

// JSON Schema for interviewTurn's structured output.
//
// FLAT root object, not a discriminated union. It was an `anyOf` at the root
// (done:false branch / done:true branch), which Anthropic accepts but the
// OpenAI-compatible strict json_schema contract -- which Groq implements --
// does not: there the root of a strict schema must be a plain
// `type: "object"`. A root union is rejected before the model ever runs, so
// every interview turn failed identically ("the interviewer lost their train
// of thought").
//
// The union is therefore expressed in the prompt and enforced in the route's
// shape check, not in the schema: every field is present on every turn, and
// the ones that do not apply are null. additionalProperties:false and
// required on every object; no length/range constraints (unsupported).
...
const INTERVIEW_TURN_SCHEMA = {
  ...
  properties: {
    ...
    next_question: { anyOf: [{ type: 'null' }, { type: 'string' }] },
    summary: { anyOf: [{ type: 'null' }, INTERVIEW_SUMMARY_SCHEMA] },
  },
  required: ['done', 'rating', 'feedback', 'next_question', 'summary'],
  additionalProperties: false,
};
...
const DIAGNOSE_PROMPT =
  ...
  'The message begins with problems ALREADY DETECTED by deterministic checks. ' +
  'Those are handled — never restate them, never mention the same section for ' +
  'the same reason. Report ONLY what a rule cannot see.\n\n' +
  'CHECK FOR (judgement only): a vague skills line with no concrete tools; ' +
  'bullets with no active verb or no outcome; a profile that does not name the ' +
  'target role in the words a recruiter would search for; claims a reader would ' +
  'doubt; evidence that is present but buried.\n\n' +
  'RULES:\n' +
  '- AT MOST 3 flags, most severe first. Fewer is correct when the deterministic checks already cover the CV — do not pad.\n' +
  '- SEVERITY IS FIXED BY THIS TABLE, not by judgement. The same problem must ' +
  'score the same every time, because the candidate is told to fix flags and ' +
  're-check:\n' +
  '  high — no name, or no email/phone at all; evidence of tables, columns or ' +
  'graphics that break text-order parsing; the CV is unreadable as plain text.\n' +
  '  medium — one contact method missing when another is present; missing or ' +
  'inconsistent dates; a non-standard section heading; a section a reviewer ' +
  'expects and cannot find.\n' +
  '  low — wording: vague skills, bullets without an active verb, a profile ' +
  'that does not name the target role, repetition.\n' +
  ...
  '- NO FIX MAY REQUIRE THE CANDIDATE TO STATE SOMETHING THE CV DOES NOT ' +
  'EVIDENCE. This is a school leaver: telling them to add "portfolio ' +
  'management" or "financial analysis" to sound relevant is telling them to lie ' +
  'on a document an employer will interview them about. Two legitimate shapes ' +
  'for a keyword fix, and no others:\n' +
  '  (a) a rewording — they already did the thing, in other words. Say which ' +
  'line to reword and to what ("\'worked the till\' -> \'point-of-sale (POS) ' +
  'and cash handling\'").\n' +
  '  (b) an earned gap — the term is missing because the experience is. Say so ' +
  'plainly and give the action that would earn it, not the words to type.\n' +
  '- Never suggest inventing or estimating a date, figure, grade or employer. ' +
  'Where one is missing, the fix is to supply the real one, and the example in ' +
  'the fix must be visibly a placeholder rather than a plausible value.\n' +
  '- The score must never be reachable by writing something untrue. If the only ' +
  'way to clear a flag is to overstate, say what the honest ceiling is instead.\n' +
  '- Banned words: leveraged, synergy, passionate, dynamic, results-driven, ' +
  'spearheaded.\n\n' +
...
const EXTRACT_PROMPT =
  ...
  '{"name":"","dob":"","address":"","email":"","phone":"","links":"",' +
  '"profile":"","school":"",' +
  '"schooldates":"","gcse":"",' +
  ...
  'Notes: "dob" = date of birth, ONLY if a complete date is explicitly stated ' +
  'in the CV text, formatted "YYYY-MM-DD" — leave "" if absent, partial (e.g. ' +
  'year only), or ambiguous, never infer one. "address" = the candidate\'s ' +
  'home address as given (as much of street/town/postcode as is present); ' +
  'leave "" if absent. "profile" = an existing personal statement / profile ' +
  'summary / "about me" section already written in the CV, taken near-' +
  'verbatim (light tidying only); leave "" if the CV has none — never write ' +
  'one from scratch. "quant" = maths/competition/achievement evidence; ' +
  '"skills" = comma-separated tools and skills; "sport" = sports or physical ' +
  'activities; "responsibility" = prefect/captain/mentor-style roles; put ' +
  'anything that fits nowhere else into "interests". "qualifications" — one ' +
  'entry per qualification found (GCSE, A-level, AS-level, BTEC, T-Level, ' +
  'National 5, Higher, Advanced Higher, or Other); set "predicted" true only ' +
  'when the CV text itself says a grade is predicted/target/expected, never ' +
  'guess.\n\n' +
...
const EXTRACT_SCHEMA = {
  ...
  required: [
    'name', 'dob', 'address', 'email', 'phone', 'links', 'profile', 'school', 'schooldates', 'gcse',

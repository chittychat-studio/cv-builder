...
function createGroqClient(apiKey, { fetchImpl, model } = {}) {
  ...
  function recordUsage(usage) {
    ...
    console.log(
      ...
      `avg=${state.avgTokensPerCall} tokens-left-this-minute=${left} ` +
      `requests-left-today=${reqLeft}`
    );
  }
  ...
  return {
     ...
     *
     * The two Groq headers measure DIFFERENT WINDOWS, and they must not be
     * compared. Groq documents this explicitly:
     *   x-ratelimit-*-requests  -> always Requests Per DAY (RPD)
     *   x-ratelimit-*-tokens    -> always Tokens Per MINUTE (TPM)
     * So tokensLeft is a burst throttle that refills in under a minute, and
     * requestsLeft is the only header-backed figure that means "today".
     * An earlier version took min() of the two and reported the result as a
     * daily allowance -- which told a student "2 uses left" one second after
     * a call, when they in fact had hundreds and the throttle cleared in 40s.
     *
     * callsLeft therefore tracks requests only. burstCallsLeft is the separate
     * per-minute figure, for "busy right now, try again shortly". Both are
     * null until the first call: a made-up number is worse than no number.
     */
    limits() {
      const avg = state.avgTokensPerCall;
      // Per-minute throttle. Refills continuously; resetsInMs.tokens says when.
      const burstCallsLeft = (state.tokensRemaining !== null && avg)
        ? Math.floor(state.tokensRemaining / avg) : null;
      // Per-day allowance. The only header that means "today".
      const callsLeft = state.requestsRemaining;
      const boundBy = callsLeft === null ? null : 'requests';
      ...
      return {
        ...
        callsLeft, boundBy,
        callsLeftWindow: 'day',
        burstCallsLeft, burstWindow: 'minute',
        observed: {
    ...
    messages: {
      async create(params = {}) {
        try {
          ...
          // Structured output refused: retry once as plain text. The callers
          // that ask for a schema (interviewTurn) already run the reply
          // through parseModelJson and validate its shape, so JSON in prose
          // still works -- less reliably, but a practice interview that runs
          // beats one that always errors.
          if (err && err.schemaRejected && params.output_config && params.output_config.format) {
            console.log(`${new Date().toISOString()} groq schema-rejected -> retrying without response_format`);
            const plain = Object.assign({}, params, {
              output_config: Object.assign({}, params.output_config, { format: undefined }),
            });
            return await send(chosenModel, plain);
          }
  ...
  async function send(modelId, params) {
        ...
        const body = {
          ...
          // Every task in this app wants consistency over variety: scoring,
          // extraction and structured output are graded twice by users, and a
          // CV rewrite that changes wording on each run is a bug, not a
          // feature. Same CV in, same CV out.
          temperature: 0,
          messages,
        };
        ...
        if (!resp.ok) {
          ...
          // A 400 that names the schema means Groq rejected our structured
          // output contract, not the prompt. Flagged so the caller can retry
          // once without it rather than failing the whole turn: the OpenAI
          // strict-schema rules Groq implements are narrower than Anthropic's
          // and differ again between models, so this must degrade, not break.
          err.schemaRejected = resp.status === 400
            && /response_format|json_schema|schema/i.test(String(detail));
          // A 429 is the free tier's tokens-per-minute ceiling, not a failure.
          // Flagged so routes can say "busy, try again shortly" with a real
          // wait instead of reporting a generic error the user cannot act on.
          if (resp.status === 429) {
            err.rateLimited = true;
            const ra = Number(resp.headers && resp.headers.get && resp.headers.get('retry-after'));
            err.retryAfterMs = Number.isFinite(ra) ? ra * 1000
              : (state.tokensResetMs || state.requestsResetMs || null);
          }
          // Groq's own error text (no CV content) — useful in the deploy log.
          console.log(`${new Date().toISOString()} groq-error status=${resp.status} ${String(detail).slice(0,200).replace(/\s+/g,' ')}`);
          throw err;
        }
